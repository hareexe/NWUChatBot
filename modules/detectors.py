def compute_detectors(user_tokens: set) -> dict:
    ...
    # --- 2. COMPLEX DETECTORS (DEFINING INTERMEDIATE FLAGS FIRST) ---
    
    # Leadership/President Flags
    ...
    is_barangan_like = any(t in user_tokens for t in {"barangan", "cresencio", "cashier", "funds", "finance"}) # LINE 35 CALCULATION
    ...
    # --- 3. COMPOSITE DETECTORS (MUST BE CALCULATED AFTER INTERMEDIATE FLAGS) ---
    ...
    # Chronology/Transition Composites
    ...
    return {
        ...
        "is_first_college_president_query": strong_first_college_president, # Reusing strong detector
        ...
        "is_barangan_query": is_barangan_like, # FIX: NameError resolved by defining it first
        ...
        "is_general_info_like": any(t in user_tokens for t in {"what", "tell", "university", "overview"}),
        ...
        # Structural/Building (simplified for readability, assuming existing complex logic)
        "is_motto_query": any(t in user_tokens for t in {"fiat", "lux", "motto"}),
        "is_logo_query": any(t in user_tokens for t in {"logo", "owl", "seal"}),
        "is_buildings_overview_query": any(t in user_tokens for t in {"buildings", "campus"}),
